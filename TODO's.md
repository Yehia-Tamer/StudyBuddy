...
10. **Multimodal document understanding** — reasoning over images/diagrams/charts within PDFs (and now PPTX) directly, rather than OCR-then-text-only. Candidate patterns expected from the Multimodal GenAI Apps course module; watch for applicable techniques while going through it. **Up next.**

## New Output Formats

11. ❌ **Mind maps / concept diagrams** — decided against, and fully removed (not just parked). Originally deprioritized because the main differentiator over `StudyPlan` (arbitrary-depth nesting vs. its flat 2-level structure) had limited payoff without a frontend to render the tree, and functionally overlapped with the working `StudyPlan` hierarchy. Migration `56671bf704cc` ("remove mind maps") subsequently dropped the `mind_maps` and `mind_map_documents` tables outright, and the many-to-many association cleanup only wired up `quiz_documents`/`study_plan_documents`/`cheat_sheet_documents`/`flashcard_documents` — `mind_map_documents` was never populated. If concept-mapping is wanted later, this is a from-scratch rebuild (new migration + schema + repository + chain + router), not a resume of parked work.
...
- ✅ **Many-to-many document associations for generated content** — added `quiz_documents`, `study_plan_documents`, `cheat_sheet_documents`, and `flashcard_documents` association tables (plain `Table()` objects, not mapped classes, since they hold no data beyond the two foreign keys), replacing the originally-planned JSON-string `document_ids` approach. Closed a real gap: `Quiz`, `StudyPlan`, and `FlashCard` previously had no way to trace which documents they were generated from, despite their generation requests already requiring `document_ids`. Repository functions for cheat sheets, quiz, study plan, and flashcards all updated to pass `documents=[...]` when constructing the parent object (caught and fixed a bug where `documents` was mistakenly attached to `QuizQuestion` instead of `Quiz`). Corresponding response schemas (`CheatSheetResponse`, `QuizResponse`, `StudyPlanResponse`, `FlashCardResponse`) all use a `model_validator(mode='before')` to transform the ORM object — extracting `document_ids` from the `documents` relationship — before Pydantic's per-field validation runs. `flashcards.document_id` (single-FK) was dropped in favor of the association table via migration `4490e1ce28fb`, bringing flashcards in line with the other three generated-content types. `mind_map_documents` was created in the same original migration but never wired up before mind maps were removed entirely (see item 11).
...
- **Cheat sheets complete; mind maps fully removed** — see item 11 for reasoning and the migration that dropped the tables.
- **Flashcards, quizzes, study plans, and cheat sheets are all consistently multi-document** now, each via its own association table and the same `model_validator`/`documents=[...]` pattern.
- **Remaining backlog is genuinely lower-priority / speculative** and out of scope for the end-of-August target: handwritten notes OCR (#9), multimodal document understanding (#10, up next), podcast-style summaries (#13), spaced repetition (#14), cross-document study plans informed by quiz performance (#15).
- **Immediate next steps before frontend work begins:** a README pass and project overview refresh (this pass), then begin scoping multimodal document understanding (#10).