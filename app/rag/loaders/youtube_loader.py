...
def fetch_transcript(url:str):
    ...
    try:
        ...
        raise YoutubeTranscriptError(f"No transcript available for the video with ID {video_id}: {e}")
    ...
    if transcript_data is None:
        raise YoutubeTranscriptError(f"No english transcript found for the video {video_id}")

    return transcript_data

def chunk_transcript_with_timestamps(transcript_data, url: str, chunk_size: int = 800, overlap_segments: int = 2) -> list[Document]:
    chunks = []
    buffer_segments = []
    buffer_len = 0
...
def load_youtube_document(url: str, chunk_size: int = 800, chunk_overlap_segments: int = 2) -> list[Document]:
