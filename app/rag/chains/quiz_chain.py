...
class QuizLLM(BaseModel):
    ...
    questions: list[QuizQuestionLLM] = Field(
        description="The questions testing the understanding of the material"
    )
    time_estimate_minutes: int = Field(
        description="An estimate of how long the entire quiz should last"
    )
...
def generate_quiz(
    user_id: int, document_ids: list[int], difficulty: str, count: int = 10
):
    ...
    for _ in range(len(API_KEYS)):
        ...
        try:
            ...
            response = chain.invoke(
                {"context": context, "count": count, "difficulty": difficulty}
            )
            return (
                response["questions"],
                response["topic"],
                response["time_estimate_minutes"],
            )
