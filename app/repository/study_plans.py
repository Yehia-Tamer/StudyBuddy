...
def generate_and_save_study_plan(user_id: int, document_ids: list[int], db: Session):
    ...
    study_plan = models.StudyPlan(
        user_id=user_id, documents=documents, title=response["title"]
    )
    db.add(study_plan)
    db.commit()
...
def get_study_plan(study_plan_id: int, user_id: int, db: Session):
    study_plan = (
        db.query(models.StudyPlan).filter(models.StudyPlan.id == study_plan_id).first()
    )
    if not study_plan:
        raise HTTPException(
...
def update_item_completion(
    study_plan_id: int, item_id: int, user_id: int, completed: bool, db: Session
):
