...
def generate_and_save_cheat_sheet(user_id: int, document_ids: list[int], db: Session):
    documents = (
        db.query(models.Document).filter(models.Document.id.in_(document_ids)).all()
    )
...
def get_cheat_sheet(cheat_sheet_id: int, user_id: int, db: Session):
    cheat_sheet = (
        db.query(models.CheatSheet)
        .filter(models.CheatSheet.id == cheat_sheet_id)
        .first()
    )
    if not cheat_sheet:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Cheat sheet not found"
        )
...
def get_cheat_sheets(user_id: int, db: Session):
    return (
        db.query(models.CheatSheet).filter(models.CheatSheet.user_id == user_id).all()
    )
