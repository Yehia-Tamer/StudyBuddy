...
def send_message(
    conversation_id: int, user_id: int, role: str, content: str, db: Session
):
    conversation = get_conversation(conversation_id, user_id, db)
    ...
    ai_answer, sources = chat_chain.ask_with_tools(
        content, user_id, conversation.document_id, history=history
    )

    ai_message = save_message(
        conversation_id, user_id, "assistant", ai_answer, db, sources=sources
    )
