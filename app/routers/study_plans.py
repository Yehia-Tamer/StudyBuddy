...
@router.post(
    "/", status_code=status.HTTP_201_CREATED, response_model=schemas.StudyPlanResponse
)
def create_study_plan(
    ...
    return study_plans.generate_and_save_study_plan(
...
@router.put(
    "/{study_plan_id}/items/{item_id}",
    status_code=status.HTTP_200_OK,
    response_model=schemas.StudyPlanItemResponse,
)
def complete_item(
    request: schemas.StudyPlanItemUpdate,
    study_plan_id: int,
    item_id: int,
    current_user: schemas.UserResponse = Depends(oauth2.get_current_user),
    db: Session = Depends(database.get_db),
):
    return study_plans.update_item_completion(
        study_plan_id, item_id, current_user.id, request.completed, db
    )
