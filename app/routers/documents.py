...
@router.post(
    "/pdf", status_code=status.HTTP_201_CREATED, response_model=schemas.DocumentResponse
)
def upload_document(
...
def upload_youtube_document(
    request: schemas.YouTubeDocumentRequest,
    current_user: schemas.UserResponse = Depends(oauth2.get_current_user),
    db: Session = Depends(database.get_db),
):
...
@router.post(
    "/web", status_code=status.HTTP_201_CREATED, response_model=schemas.DocumentResponse
)
def upload_web_document(
    request: schemas.WebDocumentRequest,
    current_user: schemas.UserResponse = Depends(oauth2.get_current_user),
    db: Session = Depends(database.get_db),
):
