...
def generate_flashcards(
    ...
    return flashcards.generate_and_save_flashcards(
        request.document_ids, current_user.id, request.count, db
    )


@router.get(
    "/", status_code=status.HTTP_200_OK, response_model=List[schemas.FlashCardResponse]
)
def get_flashcards(
    current_user: schemas.UserResponse = Depends(oauth2.get_current_user),
    db: Session = Depends(database.get_db),
...
@router.post(
    "/{flashcard_id}/answer",
    status_code=status.HTTP_200_OK,
    response_model=schemas.FlashCardAnswerResponse,
)
def answer_flashcard(
    request: schemas.FlashCardAnswerRequest,
    flashcard_id: int,
    current_user: schemas.UserResponse = Depends(oauth2.get_current_user),
    db: Session = Depends(database.get_db),
):
    return flashcards.answer_flashcard(
        flashcard_id, current_user.id, request.user_answer, db
    )
