...
@router.post(
    "/", status_code=status.HTTP_201_CREATED, response_model=schemas.QuizResponse
)
def generate_quiz(
    ...
    return quizzes.generate_and_save_quiz(
        current_user.id, request.document_ids, request.difficulty, request.count, db
    )
...
@router.get(
    "/{quiz_id}", status_code=status.HTTP_200_OK, response_model=schemas.QuizResponse
)
def get_quiz(
    quiz_id: int,
    current_user: schemas.UserResponse = Depends(oauth2.get_current_user),
...
@router.post(
    ...
    response_model=schemas.QuizGradeResponse,
)
def grade_quiz(
    request: schemas.QuizGradeRequest,
    quiz_id: int,
    current_user: schemas.UserResponse = Depends(oauth2.get_current_user),
    db: Session = Depends(database.get_db),
):
    score, results = quizzes.grade_quiz(request.answers, quiz_id, current_user.id, db)
    return {"score": score, "total": len(request.answers), "results": results}
